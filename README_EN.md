...
This mod currently targets X4 9.00 and requires:
...
2. Copy the entire `extensions/gatefall_chronicles` folder into your X4 `extensions` directory.
3. Start the game and make sure `Gatefall Chronicles` is enabled in the Extensions menu.
...
5. Scroll to the end of the list and find the `Gatefall Chronicles` entries.
6. Select the scenario you want to play.

If you have already accessed the official hidden scenarios, the process is essentially the same. Gatefall Chronicles uses that same scenario list.
...
- Get dragged into the Xenon core territory of Savage Spur and break out towards a TER outpost.
- Hold the line until the Terran Intervention Corps battleship `TER Paradigm` arrives.

Key features: frigate combat, capital-ship engine attacks, outpost defence, multi-stage cutscenes, and scored results.
...
- Infiltrate a classified database and acquire a Yaki navigation protocol capable of altering Xenon attack vectors.
...
- Protect a salvage ship while it recovers blockade intelligence from a wrecked Guppy carrier.
- Lead a Boron–Terran fleet against Xenon radar ships and neutralise the long-range artillery threat.
- Deploy a player-controlled Tactical Production Platform and advance through shipbuilding, renewable resources, and fleet organisation.
- Conduct an RTS-style offensive against Xenon power stations, shipbuilding facilities, and the M0 itself.
...
Thanks to the X4 community, the official game and DLC mission files that made technical research possible, and everyone willing to test this hidden campaign project.

If you encounter scripting, cutscene, scoring, balance, or access issues, feel free to open a GitHub issue or leave feedback on the Steam Workshop page.

Other scenarios are also planned as tributes to classic games and campaigns. `A Faint Hope` is intended as a tribute to the Starsector mission of the same name, while `Crime and Punishment` is planned as a tribute to the Mental Omega 3.3.6 mission of the same name.
